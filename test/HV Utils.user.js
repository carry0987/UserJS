    ...
    if (a !== null && a !== undefined) {
        switch (a.constructor) {
            ...
            case String:
                ...
                break;
            case Array:
                a.forEach(function(aa) { var a0 = ({ " ": "textContent", "#": "id", ".": "className", "!": "style", "/": "innerHTML" })[aa[0]]; if (a0) { e[a0] = aa.substr(1); } });
                break;
            case Object:
                var ai, av, es, esi;
                for (ai in a) { av = a[ai]; if (av && av.constructor === Object) { if (ai in e) { es = e[ai]; } else { es = e[ai] = {}; } for (esi in av) { es[esi] = av[esi]; } } else { if (ai === "style") { e.style.cssText = av; } else if (ai in e) { e[ai] = av; } else { e.setAttribute(ai, av); } } }
                break;
        }
    }
    if (f) { if (f.constructor === Function) { e.addEventListener("click", f, false); } else if (f.constructor === Object) { var fi; for (fi in f) { e.addEventListener(fi, f[fi], false); } } }
    if (p) { if (p.nodeType === 1 || p.nodeType === 11) { p.appendChild(e); } else if (Array.isArray(p)) { if (["beforebegin", "afterbegin", "beforeend", "afterend"].includes(p[1])) { p[0].insertAdjacentElement(p[1], e); } else if (!isNaN(p[1])) { p[0].insertBefore(e, p[0].childNodes[p[1]]); } else { p[0].insertBefore(e, p[1]); } } }
    return e;
}

function time_format(t, o) {
    t = Math.floor(t / 1000);
    var h = Math.floor(t / 3600).toString().padStart(2, "0"),
        m = Math.floor(t % 3600 / 60).toString().padStart(2, "0"),
        s = (t % 60).toString().padStart(2, "0");
    return !o ? h + ":" + m + ":" + s : o === 1 ? h + ":" + m : o === 2 ? m + ":" + s : "";
}

function object_sort(o, x = []) {
    var keys = Object.keys(o),
        index_object = {};
    x.forEach((e, i) => { index_object[e] = i + 1; });
    keys.sort((a, b) => {
        var _a = index_object[a] || x.length + 1,
            _b = index_object[b] || x.length + 1;
        return _a - _b || (a < b ? -1 : 1);
    });
    keys.forEach(e => {
        var v = o[e];
        delete o[e];
...
function substring(t, s, e, d) {
    ...
    if (typeof s !== "string") {
        so = s[1];
        if (s[2]) { si = t.indexOf(s[2]); } s = s[0];
        so = so === true ? s.length : so;
    }
    si = t.indexOf(s, si);
    if (typeof e !== "string") {
        eo = e[1];
        if (e[2]) { ei = t.indexOf(e[2]); } e = e[0];
        eo = eo === true ? e.length : eo;
    }
    ei = e ? t.indexOf(e, si > ei ? si : ei) : t.length;
    if (si === -1 || ei === -1) { return null; } t = t.substring(si + so, ei + eo);
    if (d) {
...
function confirm_event(n, e, m, c, f) {
    if (!n) { return; }
    var a = n.getAttribute("on" + e);
    n.removeAttribute("on" + e);
    n.addEventListener(e, function(e) {
        if (!c || c()) {
            if (confirm(m)) { if (f) { f(); } } else {
                e.preventDefault();
                e.stopImmediatePropagation();
            }
        }
    }, true);
    n.setAttribute("on" + e, a);
}

function play_beep(s = { volume: 0.2, frequency: 500, time: 0.5, delay: 1 }) {
    if (!s.volume) { return; }
    var c = new window.AudioContext(),
        o = c.createOscillator(),
        g = c.createGain();
    o.type = "sine";
    o.frequency.value = s.frequency;
    g.gain.value = s.volume;
    o.connect(g);
    g.connect(c.destination);
    o.start(s.delay);
    o.stop(s.delay + s.time);
}
...
function popup(t, s) {
    var r = function(e) {
            e.stopImmediatePropagation();
            e.preventDefault();
            if (e.which === 1 || e.which === 13 || e.which === 27 || e.which === 32) {
                w.remove();
                document.removeEventListener("keydown", r);
            }
        },
        w = $element("div", document.body, ["!position:fixed;top:0;left:0;width:1236px;height:702px;padding:3px 100% 100% 3px;background-color:rgba(0,0,0,.6);z-index:1001;display:flex;justify-content:center;align-items:center"]),
        d = $element("div", w, ["/" + t, "!min-width:400px;min-height:100px;max-width:100%;max-height:100%;padding:10px;background-color:#fff;border:1px solid #333;cursor:pointer;display:flex;flex-direction:column;justify-content:center;font-size:10pt;color:#000;" + (s || "")], r);
    document.addEventListener("keydown", r);
    return d;
}

function popup_text(m, s, b = []) {
    var v, l;
    if (typeof m === "string") {
        v = m;
        l = m.split("\n").length;
    ...
    var w = $element("div", document.body, ["!position:fixed;top:0;left:0;width:1236px;height:702px;padding:3px 100% 100% 3px;background-color:rgba(0,0,0,.6);z-index:1001;display:flex;justify-content:center;align-items:center"]),
        d = $element("div", w, ["!border:1px solid #333;padding:5px;background-color:#fff"]),
        t = $element("textarea", d, { value: v, spellcheck: false, style: "display:block;margin:0 0 5px;font-size:9pt;line-height:1.5em;height:" + (l * 1.5 + 1) + "em;" + (s || "") });
    b.forEach(o => { $element("input", d, { type: "button", value: o.value }, () => o.click(w, t)); });
    $element("input", d, { type: "button", value: "Close" }, function() { w.remove(); });
    return w;
}
...
if (location.search) {
    location.search.substr(1).split("&").forEach(function(q) {
        q = q.split("=", 2);
        _query[q[0]] = decodeURIComponent(q[1].replace(/\+/g, " "));
    });
}
...
if ($id("navbar")) {
    ...
    $equip.bbcode = function(eq) {
        function rainbow(text) {
            var c = ["#f00", "#f90", "#fc0", "#0c0", "#09f", "#00c", "#c0f"],
                l = c.length;
        ...
        if (eq.info.quality === "Peerless") {
            quality.code = rainbow(quality.code);
            quality.bold = true;
    ...
    var $prices = {
        ...
        selector: function() {
            $prices.init();
            var selector = $element("select", null, null, {
                change: function() {
                    selector.blur();
                    $prices.edit(selector.value);
                }
            });
...
`);
    ...
    $dfct.hover = function() {
        if (!$dfct.sub) {
            ...
            $dfct.selector = $element("select", $dfct.sub, { size: $dfct.list.length, style: "width:80px" }, {
                change: function() {
    ...
    $dfct.change = function(value) {
        ...
        $ajax.add("GET", "/?s=Character&ss=se", null, function(r) {
            ...
            if (input) {
                ...
                $ajax.add("POST", "/?s=Character&ss=se", post, function(r) {
                    var doc = substring(r.responseText, "<body", ["</body>", true], true);
                    $dfct.set_button(doc);
                }, function() { $dfct.button.textContent = "(D:ERROR)"; });
    ...
    $persona.hover = function() {
        ...
        if (!$persona.sub) {
            ...
            $persona.selector_p = $element("select", $persona.sub, { size: json.plist.length - 1, style: "width:100px" }, {
                change: function() {
            ...
            $persona.selector_e = $element("select", $persona.sub, { size: json.plist[json.pidx].elist.length - 1, style: "width:100px" }, {
                change: function() {
    ...
    `);
        ...
        ["lt", "la"].forEach(function(ss) {
            ...
            var div = $element("div", _bottom.div, [".hvut-lt-div"]),
                ...
                hide_span = $element("span", div, "close", function() {
                    json.hide = true;
...
`);
            ...
            $element("input", _ch.node.div, { type: "button", value: "Simulate", style: "margin-right:50px" }, function() {
                $qs("img[onclick*='do_attr_post']").style.visibility = "hidden";
                _ch.node.sub.classList.toggle("hvut-none");
        ...
        `);
                ...
                $element("label", ul).append($element("input", null, { type: "checkbox", checked: edb_infusion }, function() {
                    $persona.set_value("e", "infusion", this.checked);
                    _eq.mage_stats();
                }), "Infusion");
                $element("label", ul).append($element("input", null, { type: "checkbox", checked: mit_imperil }, function() {
                    $persona.set_value("e", "imperil", this.checked);
                    _eq.mage_stats();
                }), "Imperil");
    ...
    `);
                    ...
                    table: function() {
                        ...
                        var frag = $element();
                        $element("tr", frag, ["/<td data-desc='Player Level'>" + (reverse ? "&#x25BC;" : "&#x25B2;") + " Lv</td><td data-desc='Required Ability Point'>AP</td><td data-desc='Required Ability Boost (AP - Lv)'>AB</td><td>Abilities</td>"], function() {
                            _ab.calc.reverse = !reverse;
                            _ab.calc.table();
                        });
...
`);
                        ...
                        if (settings.equipInventory && _query.s === "Character" && _query.ss === "in") {
                            ...
                            _in.get_pr = function(eq) {
                                eq.node.pr.value = "...";
                                var ifr = $element("iframe", document.body, { style: "display:none" }, {
                                    load: function() {
                            ...
                            _in.bbcode = function() {
                                ...
                                list.sort(function(a, b) {
                                    ...
                                    var sortkey =
                                        (a.info.category === "One-handed Weapon" || a.info.category === "Two-handed Weapon") ? ["suffix", "quality", "prefix"] :
                                        a.info.category === "Staff" ? ["prefix", "suffix", "quality"] :
                                        a.info.category === "Shield" ? ["quality", "suffix", "prefix"] : ["suffix", "slot", "quality", "prefix"];
    ...
    `);
        ...
        ` + (settings.equipInventoryShop ? "" : ".hvut-in-price, .hvut-in-note, .hvut-in-pr {display:none}"));
                                ...
                                $element("br", _in.node.side);
                                $element("input", [$element("label", _in.node.side, "All"), "afterbegin"], { type: "checkbox" }, function() {
                                    var c = this.checked;
                                    _in.list.forEach(eq => { eq.node.check.checked = c; });
                                });
                                $element("br", _in.node.side);
                                $element("input", [$element("label", _in.node.side, "Tradeables"), "afterbegin"], { type: "checkbox" }, function() {
                                    var c = this.checked;
...
`);
    ...
    `);
                                    ...
                                    if (_query.filter || !settings.equipmentShopIntegration) {
                                        ...
                                        $element("input", _es.div, { type: "button", value: "Hide Equipment", style: "position:absolute;left:100%;margin-left:25px;width:125px" }, function() {
                                            if ($id("csp").classList.contains("hvut-hide-on")) {
                                                $id("csp").classList.remove("hvut-hide-on");
                                                this.value = "Hide Equipment";
                                            } else {
                                                $id("csp").classList.add("hvut-hide-on");
                                                this.value = "Show Equipment";
                                            }
                                        });
...
`);
                                        ...
                                        $id("csp").classList.add("hvut-hide-on");
                                        $element("input", $id("itshop_outer"), { type: "button", value: "Show Items", style: "position:absolute;top:75px;right:220px;width:90px;z-index:3" }, function() {
                                            if ($id("csp").classList.contains("hvut-hide-on")) {
                                                $id("csp").classList.remove("hvut-hide-on");
                                                this.value = "Hide Items";
                                            } else {
                                                $id("csp").classList.add("hvut-hide-on");
                                                this.value = "Show Items";
                                            }
                                        });
...
`);
                                                    ...
                                                    $qsa("#slot_pane > div").forEach(function(div, i) {
                                                        ...
                                                        mob.node.hunger = $element("div", hungerdiv, [" " + mob.hunger, ".hvut-ml-feed"], function(e) {
                                                            _ml.main.feed(mob, "food");
                                                            e.stopPropagation();
                                                        });
                                                        mob.node.morale = $element("div", moralediv, [" " + mob.morale, ".hvut-ml-feed"], function(e) {
                                                            _ml.main.feed(mob, "drugs");
                                                            e.stopPropagation();
                                                        });

                                                        mob.node.win = $element("div", div, "-", function(e) {
                                                            _ml.main.feed(mob);
                                                            e.stopPropagation();
                                                        });
    ...
    `);
                                                            ...
                                                            if (ct_next === Math.ceil(1 + Math.pow(ct_slot, 1.2))) {} else if (ct_next === Math.ceil(1 + Math.pow(ct_slot / 2, 1.2))) {
                                                                ct_slot = ct_slot / 2;
                                                                ct_gs = true;
                                                            } else {
                                                                ct_slot = 0;
    ...
    `);
        ...
        <tr><td>2250</td><td>Power Level cap reached<br>Morale drain reduced by 6x</td></tr>
                                                    ...
                                                    $element("input", _ml.main.buttons, { type: "button", value: "Reset Log" }, function() {
                                                ...
                                                if (settings.shrine && _query.s === "Bazaar" && _query.ss === "ss") {
                                                    ...
                                                    _ss.view_log = function() {
                                                        ...
                                                        $element("input", $element("div", div, ["!margin-top:20px;padding:10px;border-top:1px solid #999"]), { type: "button", value: "Reset Log" }, function() {
                                                            if (confirm("Shrine reward log will be deleted.\n\nContinue?")) {
                                                                deleteValue("ss_log");
                                                                location.href = location.href;
                                                            }
                                                        });
    ...
    #inv_item .itemlist td:nth-child(2) {width:50px}
    #inv_item .itemlist td:nth-child(3) {width:140px; text-align:left}
...
`);
                                                    ...
                                                    $id("csp").classList.add("hvut-hide-on");
                                                    $element("input", _ss.node.buttons, { type: "button", value: "Show Items", style: "width:90px" }, function() {
                                                        if ($id("csp").classList.contains("hvut-hide-on")) {
                                                            $id("csp").classList.remove("hvut-hide-on");
                                                            this.value = "Hide Items";
                                                        } else {
...
`);
                                                            ...
                                                            _mm.create_page = function(frag, page) {
                                                                ...
                                                                Array.from(frag.rows).forEach(function(row, i) {
                                                                    ...
                                                                    $element("a", $element("td", data.page.tr), { href: "/?s=Bazaar&ss=mm&filter=" + _mm.filter + "&mid=" + mid + "&page=" + page }, function(e) {
                                                                        e.preventDefault();
                                                                        _mm.load_mail(mid, true);
                                                                    });
                                                                ...
                                                                _mm.db.conn().openCursor().onsuccess = function(e) {
                                                                    ...
                                                                    if (cursor) {
                                                                        ...
                                                                        list.forEach(function(mm) {
                                                                            ...
                                                                            $element("td", tr);
                                                                            $element("a", $element("td", tr), { href: "/?s=Bazaar&ss=mm&filter=" + db.filter + "&mid=" + db.mid }, function(e) {
                                                                                e.preventDefault();
                                                                                _mm.load_mail(db.mid, true);
                                                                            });
                                                                            $element("td", tr);
                                                                            $element("td", tr);
                                                                            $element("td", tr);
                                                            ...
                                                            _mm.itemshop = function(mid, items) {
                                                                ...
                                                                if (!status.taken) {
                                                                    _mm.load_mail(mid, true, "action=attach_remove&mmtoken=" + data.mail.mmtoken + "&action_value=0", function() {
                                                                        status.taken = true;
                                                                        _mm.itemshop();
                                                                    });
                                                                    _mm.log("[Item Shop Request]", 1);
...
`);
                                                            ...
                                                            $element("input", _mm.node.search_div, { type: "button", value: "Close" }, function() {
                                                                if (_mm.search.div) {
                                                                    _mm.search.div.remove();
                                                                    _mm.search.div.innerHTML = "";
                                                                }
                                                            });
                                                            ...
                                                            _mm.db.open(function() {
                                                                _mm.create_page($id("mmail_list"), _mm.page);
                                                        ...
                                                        if (settings.lottery && _query.s === "Bazaar" && (_query.ss === "lt" || _query.ss === "la")) {
                                                            ...
                                                            if (settings.showLottery && $qs("img[src$='lottery_next_d.png']")) {
                                                                ...
                                                                $element("input", _lt.label, { type: "checkbox", checked: !_lt.json.hide, style: "vertical-align:middle;top:0" }, {
                                                                    change: function() {
                                                                        _lt.json.hide = !this.checked;
...
`);
                                                                        ...
                                                                        $equip.list($id("item_pane"), 1).forEach(function(eq) {
                                                                            eq.node.div.addEventListener("click", function() {
                                                                                _iw.json[_iw.filter] = eq.info.eid;
                                                                                setValue("iw_json", _iw.json);
                                                                                _iw.show_potency(eq);
                                                                            });
