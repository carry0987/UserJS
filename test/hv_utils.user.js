...
topMenuLinks : ["Character", "Equipment", "Item Inventory", "Equip Inventory", "Equipment Shop", "Item Shop", "Monster Lab", "The Shrine", "MoogleMail", "The Arena", "The Tower", "Ring of Blood", "GrindFest", "Item World"],
...
character : true,
    ...
    characterReallocate : true, // reallocate Primary Attributes
...
equipEnchant : true, // Equipment Enchant and Repair Pane
    ...
    equipEnchantInventory : ["Health Draught", "Mana Draught", "Spirit Draught", "Health Potion", "Mana Potion", "Spirit Potion", "Health Elixir", "Mana Elixir", "Spirit Elixir"],
    equipEnchantInventoryCheck : 200, // warn when the quantity of each item is less than this number
    equipEnchantCheckArmors : false,

arena : true, // integrate page 1 and 2
ringofblood : true,
grindfest : true,
...
itemPrice : {
    ...
    "Materials" : { // Monster Lab, Upgrade and Salvage
        ...
        "Crystallized Phazon": 250000,
        "Shade Fragment": 3000,
        "Repurposed Actuator": 30000,
        "Defense Matrix Modulator": 3000,

        "Binding of Slaughter": 60000,
        "Binding of Balance": 1000,
        "Binding of Isaac": 1000,
        "Binding of Destruction": 50000,
        ...
        "Binding of the Raccoon": 5000,
        "Binding of the Cheetah": 30000,
        "Binding of the Turtle": 1000,
        "Binding of the Fox": 30000,
        "Binding of the Owl": 30000,

        "Binding of Surtr": 100,
        "Binding of Niflheim": 100,
        "Binding of Mjolnir": 100,
        "Binding of Freyr": 100,
        "Binding of Heimdall": 100,
        "Binding of Fenrir": 100,

        "Binding of the Elementalist": 100,
        "Binding of the Heaven-sent": 100,
        "Binding of the Demon-fiend": 100,
...
function substring(t,s,e,d){var si=0,ei=0,so=0,eo=0;if(typeof s!=="string"){so=s[1];if(s[2]){si=t.indexOf(s[2]);}s=s[0];so=so===true?s.length:so;}si=t.indexOf(s,si);ei=si;if(typeof e!=="string"){eo=e[1];if(e[2]){ei=t.indexOf(e[2],ei);}e=e[0];eo=eo===true?e.length:eo;}ei=e?t.indexOf(e,ei):t.length;if(si===-1||ei===-1){return null;}t=t.slice(si+so,ei+eo);if(d){d=document.createElement("template");d.innerHTML=t;t=d.content;}return t;}
...
$equip.names = getValue("equip_name",{});
...
layer : function(extended) {
    ...
    if(custom_div) {
        eq.info.customname = Array.from(custom_div.firstElementChild.children).map(d=>d.textContent||" ").join("");
    }
...
edit : function(tag,callback) {
    ...
    {value:"Default",click:function(w,t){
        var json = settings.itemPrice[tag],
            a = [];
...
GM_addStyle(`
    ...
    .hvut-scrollbar-none > option {margin:0; border:0; padding:3px; font-size:9pt}
    ...
    document.addEventListener("keydown",function(e){
        ...
        if(div) {
            if(e.which === 67) { // C
                ...
                prompt("Forum Link:","[url="+location.origin+location.pathname+"equip/"+div.dataset.eid+"/"+div.dataset.key+"]"+div.textContent+"[/url]");
...
GM_addStyle(`
    ...
    .hvut-quick > a::after {content:attr(data-text); display:none; position:absolute; top:100%; left:0; margin-top:2px; margin-left:0; line-height:1.6em; padding:1px 4px; background-color:#fff; color:#930; border:1px solid; font-size:10pt; font-weight:normal; pointer-events:none}
...
_top.menu = {
    ...
    "Switch to Isekai" : {s:"Character",href:"/isekai/",isekai:false},
    "Switch to Persistent" : {s:"Character",href:"/",isekai:true},

    "Equipment Shop" : {s:"Bazaar",ss:"es"},
    "Item Shop" : {s:"Bazaar",ss:"is"},
    "Item Shop Bot" : {s:"Bazaar",ss:"ib",isekai:false},
...
_top.init = function() {
    ...
    Object.keys(_top.menu).forEach(function(t){
        var m = _top.menu[t];
        if(!_isekai && m.isekai===true || _isekai && m.isekai===false) {
            return;
        }
        $element("a", $element("li",menu_ul[m.s]), {textContent:t,href:m.ss?"?s="+m.s+"&ss="+m.ss:m.href});
    });
    /* monsterbation */
    if($id("mbsettings")) {
    ...
    if(!_isekai) {
        ...
        $element("form",sub,{method:"POST"},{submit:function(e){if(settings.confirmStaminaRestorative&&!confirm("Are you sure you want to use a Caffeinated Candy or Energy Drink?")){e.preventDefault();}}}).append(
            ...
            $element("input",null,{type:"submit",value:"USE RESTORATIVE",disabled:_player.stamina>settings.disableStaminaRestorative,style:"font-size:10pt;font-weight:bold;height:auto;padding:3px 15px"})
        );
    ...
    if(_player.level !== 500) {
        ...
        $element("div",sub,["!width:299px;height:8px;margin:0 auto; border:1px solid;background-color:#fff;background-image:linear-gradient(to right, #930 0, transparent 1px),linear-gradient(to right, #9cf, #9cf);background-position:-1px 0, 0 0;background-size:30px, "+exp.p+"%;background-repeat:repeat, no-repeat"]);
    }
};
...
settings.topMenuLinks.forEach(function(t){
    var m = _top.menu[t];
    if(!m || !m.ss || !_isekai && m.isekai===true || _isekai && m.isekai===false) {
        return;
    }
    var text = m.ss.toUpperCase(),
        href = "?s="+m.s+"&ss="+m.ss;
    if(m.s==="Bazaar" && m.ss==="mm") {
        if(_top.mm) {
            text = "["+_top.mm+"]";
            GM_addStyle(`
    ...
    $element("a",_top.node.quick,{textContent:text,dataset:{text:t},href:href});
});

//$element("div",_top.node.div,[".hvut-spaceholder"]); // spaceholder

_top.node.stamina = $element("div",_top.node.div,["!width:80px","/<span>Stamina: "+_player.stamina+"</span>"]);
...
$dfct.hover = function(){
    if(!$dfct.sub) {
        ...
        $dfct.selector = $element("select",$dfct.sub,{size:$dfct.list.length,className:"hvut-scrollbar-none",style:"width:70px"},{change:function(){
...
$persona.hover = function(){
    ...
    if(!$persona.sub) {
        ...
        $persona.selector_p = $element("select",$persona.sub,{size:json.plist.length-1,className:"hvut-scrollbar-none",style:"width:100px"},{change:function(){$persona.selector_p.disabled=true;$persona.change_p($persona.selector_p.value);}});
        ...
        $persona.selector_e = $element("select",$persona.sub,{size:json.plist[json.pidx].elist.length-1,className:"hvut-scrollbar-none",style:"width:100px"},{change:function(){$persona.selector_e.disabled=true;$persona.change_e($persona.selector_e.value);}});
        json.plist[json.pidx].elist.forEach(function(e,i){if(!i){return;}$element("option",$persona.selector_e,{value:i,text:e.name||"Set "+i});});
        $persona.selector_e.value = json.eidx;
    }
};
...
if(settings.showEquipSlots===2 || settings.showEquipSlots===1 && _query.s==="Battle") {
    ...
    $ajax.add("GET","?s=Character&ss=in",null,function(r){
        ...
        var c = parseInt(exec[3]) - parseInt(exec[1]) - (parseInt(exec[2])||0);
        if(c < 150) {
...
// Primary Attributes
if(settings.characterReallocate) {

_ch.allocate = function(r) {
    var json = getValue("ch_allocate");
    if(!json) {
        return;
    }
    if(!r || !r.responseText) {
        $ajax.add("GET","?s=Character&ss=ch",null,_ch.allocate,_ch.allocate);
        return;
    }

    try {
        var html = r.responseText,
            persona = substring(html,'<select name="persona_set"',['</select>',true],true).firstElementChild.value,
            attr_current = JSON.parse(/var attr_current = (\{.*?\});/.test(html) && RegExp.$1),
            doovers = JSON.parse(/var doovers = (\d+)/.test(html) && RegExp.$1);
        if(json.done) {
            if(json.before && json.before!==persona) {
                $ajax.add("POST","?s=Character&ss=ch","persona_set="+json.before,_ch.allocate,_ch.allocate);
            } else {
                if(_ch.allocate.popup) {
                    _ch.allocate.popup.click();
                    _ch.allocate.popup = null;
                }
                deleteValue("ch_allocate");
                popup("Reallocating Persona #"+json.persona+" Completed!");
            }
            return;
        }
        if(!doovers) {
            if(json.before && json.before!==persona) {
                ...
                json.before = null;
                setValue("ch_allocate",json);
                setTimeout(_ch.allocate,1000*60*10);
            }
            return;
        }
        if(json.persona !== persona) {
            json.before = persona;
            setValue("ch_allocate",json);
            if(!_ch.allocate.popup) {
                _ch.allocate.popup = popup("Reallocating Persona #"+json.persona+"...");
            }
            $ajax.add("POST","?s=Character&ss=ch","persona_set="+json.persona,_ch.allocate,_ch.allocate);
            return;
        }
        var key = ["str","dex","agi","end","int","wis"].find(function(key){
            if(json[key] < attr_current[key]) {
                return true;
            }
        });
        if(key) {
            $ajax.add("POST","?s=Character&ss=ch","attr_apply=1&"+key+"_delta="+Math.max(-doovers,json[key]-attr_current[key]),_ch.allocate,_ch.allocate);
        } else {
            json.done = true;
            setValue("ch_allocate",json);
            _ch.allocate(r);
        ...
        setTimeout(_ch.allocate,1000*60);
    }
};
...
if(settings.showLottery) {
    ...
    ["lt","la"].forEach(function(ss){
        ...
        $ajax.add("GET","/?s=Bazaar&ss="+ss,null,function(r){
            ...
            if(!id || id===json.id) {
                if(html.includes('<div id="battle_top">')) {
...
_ch.init_simulate = function() {
    ...
    _ch.node.ass = $element("input",$element("label",_ch.node.div,"Assimilator"),{type:"number",value:_ch.ass,min:0,max:25,style:"width:40px"},{input:_ch.calc_prof});
    _ch.calc_prof();
};
...
_ch.calc_prof = function() {
    ...
    Object.keys(_ch.prof).forEach(function(name){
        var prof = _ch.prof[name];
        if(!prof.inited) {
            prof.inited = true;
            prof.current = parseFloat(prof.tr.cells[1].textContent);
            prof.exp = _ch.get_exp(prof.current);
            prof.tr.cells[1].textContent = prof.current;
            $element("td",prof.tr);
            $element("td",prof.tr);
        }
        prof.level = _ch.get_level(prof.exp+prof_gain,prof.current);
        prof.tr.cells[2].textContent = "+" + (prof.level-prof.current).toFixed(3);
        prof.tr.cells[3].textContent = prof.level.toFixed(3);
    });
};
...
_ch.init_allocate = function() {
    ...
    var json = getValue("ch_allocate");
    if(!json) {
        _ch.node.allocate = $element("input",_ch.node.div,{type:"button",value:"Set Primary Attributes Reallocation"},_ch.set_pa);
        $element("input",_ch.node.div,{type:"button",value:"Reset"},_ch.reset_pa);
    } else if(json.persona === _ch.persona) {
        _ch.node.allocate = $element("input",_ch.node.div,{type:"button",value:"Set Primary Attributes Reallocation"},_ch.set_pa);
        $element("input",_ch.node.div,{type:"button",value:"Reset"},_ch.reset_pa);
        _ch.get_pa();
    } else {
        _ch.node.allocate = $element("input",_ch.node.div,{type:"button",value:"Currently Reallocating Persona #"+json.persona,disabled:true});
    }
};

_ch.get_pa = function() {
    var json = getValue("ch_allocate");
    if(!json || json.persona !== _ch.persona) {
        return;
    }
    var delta = 0;
    _ch.attr_keys.forEach(function(key){
        _window.attr_delta[key] = json[key] - _window.attr_current[key];
        _window.update_usable_exp();
        _window.update_display(key);
        if(_window.attr_delta[key] < 0) {
            delta -= _window.attr_delta[key];
        }
    });
    _ch.node.allocate.value = "Reallocating: " + (delta<10?delta*2.4+" hour(s)":delta/10+" day(s)") + " left";
};

_ch.set_pa = function() {
    var json = {persona:_ch.persona};
    var delta = 0;
    _ch.attr_keys.forEach(function(key){
        json[key] = _window.attr_current[key] + _window.attr_delta[key];
        if(_window.attr_delta[key] < 0) {
            delta -= _window.attr_delta[key];
        }
    });
...
_ch.reset_pa = function() {
    deleteValue("ch_allocate");
    _ch.attr_keys.forEach(function(key){
        _window.attr_delta[key] = 0;
        _window.update_usable_exp();
        _window.update_display(key);
    });
    _ch.node.allocate.value = "Set Primary Attributes Reallocation";
};

GM_addStyle(`
    #attr_table tr:last-child > td {padding-top:10px !important}
    .hvut-ch-div {position:absolute; margin:-25px 0 0 40px; text-align:left}
    .hvut-ch-div > input {margin:0 5px}
...
if(settings.characterReallocate) {
    $element("input",_ch.node.div,{type:"button",value:"Reallocator"},_ch.init_allocate);
}
...
_eq.mage_stats = function() {
    if(!_eq.mage_stats.inited) {
        ...
        GM_addStyle(`
            ...
            .hvut-eq-chart {position:relative; display:flex; flex-wrap:wrap; width:620px; margin:0 auto; padding:10px 15px; overflow:hidden; border:1px solid; font-size:10pt; text-align:left; white-space:nowrap}
            .hvut-eq-option {width:100%; margin-bottom:5px; padding-bottom:5px; border-bottom:1px solid}
    ...
    $element("label",ul).append($element("input",null,{type:"checkbox",checked:resist_dfct},function(){_eq.mage_stats();}), "PFUDOR");
    ...
    $element("label",ul).append($element("input",null,{type:"checkbox",checked:mit_day},function(){$persona.set_value("e","day",this.checked);_eq.mage_stats();}), "Day of the Week");
...
_eq.equipset_code = function() {
    popup_text(_eq.list.map(eq=>"[url="+location.origin+location.pathname+"equip/"+eq.info.eid+"/"+eq.info.key+"]"+eq.info.name+"[/url]"),"width:800px;max-height:500px;white-space:pre");
};
...
_eq.prof = {
    ...
    init : function() {
        ...
        GM_addStyle(`
            ...
            .hvut-eq-prof > div {width:540px; margin:5px 0 20px}
            ...
            .hvut-eq-equip > div {margin:1px 0; height:22px; line-height:22px}
            ...
            .hvut-eq-equiplist > div {margin:1px 0; height:22px; line-height:22px}
            .hvut-eq-equiplist > div:first-child {border:1px solid}
            .hvut-eq-equiplist > div > *:nth-child(1) {width:110px; padding-left:5px}
        ...
        var player = $element("div",node.div,[".hvut-eq-player"],{input:function(){_eq.prof.change("player");}});
        ...
        $element("h4",node.div,"Equipment");
        node.equip = $element("div",node.div,[".hvut-eq-equip"]);
        $element("div",node.equip,["/<span>Type</span><span>sb</span><span>level</span><span>pxp</span><span>base</span><span></span><span>pmax</span><span>upgrade</span><span>forged</span><span>scaled</span><span></span>"]);
...
if(_query.equip_slot) {
    ...
    GM_addStyle(`
        ...
        #hvut-eq-buttons > input {margin-right:10px}
    `);
...
init : function() {
    ...
    GM_addStyle(`
        .hvut-ab-calc {display:flex; position:absolute; top:27px; left:0; width:100%; height:675px; justify-content:center; align-items:center; background-color:#EDEBDF; z-index:9; text-align:left; color:#000}
        .hvut-ab-calc > div {margin:0 10px; height:616px}
        .hvut-ab-calc > div:nth-child(3) {overflow:hidden auto}

        .hvut-ab-buttons > input {display:block; margin:5px; width:120px; padding:3px 5px}
        ...
        .hvut-ab-point {position:absolute; top:0; right:0; width:14px; padding:1px; text-align:center; background-color:rgba(0,0,0,0.8); color:#fff; font-size:9pt}
        ...
        .hvut-ab-category {display:inline-block; width:130px; margin-left:10px; font-size:11pt; font-weight:bold; vertical-align:middle}
        ...
        .hvut-ab-table tr:first-child > td {height:36px; background-color:#fff; font-weight:bold; text-align:center; cursor:pointer}
        .hvut-ab-table td {border:1px solid #000; padding:2px 5px}
        ...
        .hvut-ab-nolevel {background-color:#fff}
        .hvut-ab-noab {color:#999}
...
table : function() {
    ...
    filter.forEach(function(d){
        ...
        $element("td",tr,d.sum);
        $element("td",tr,[" "+d.ab,d.ab<=0?".hvut-ab-noab":""]);
...
GM_addStyle(`
    ...
    .hvut-ab-unslotted::before {content:"unslotted"; display:inline-block; margin-bottom:2px; padding:1px 3px; border-radius:2px; background-color:#900; color:#fff; font-size:10pt}
`);
...
$qsa("#ability_treepane > div").forEach(function(div){
    ...
    if(ab.level && !ab.slotted) {
        div.firstElementChild.firstElementChild.classList.add("hvut-ab-unslotted");
    }
});

$element("input",$id("mainpane"),{type:"button",value:"Ability Calculator",style:"position:absolute;top:45px;left:15px;width:80px;padding:5px;white-space:normal"},function(){_ab.calc.run();});
...
GM_addStyle(`
    ...
    .itemlist > tbody > tr > td:last-child {width:65px}
    ...
    .hvut-it-side input[type='button'] {margin:3px 0; padding:3px; white-space:normal}
...
$qs(".itemlist").classList.remove("nosel");
        ...
        if(eq.info.tier < 10 || $equip.names[eq.info.eid]) {
            ...
            $ajax.add("GET","equip/"+eq.info.eid+"/"+eq.info.key,null,function(r){
                var doc = substring(r.responseText,"<div","</body>",true),
                    e = $equip.parse.layer($id("equip_extended",doc)),
                    equipname = e.info.name;
                if(equipname && equipname !== eq.info.name) {
                    eq.info.customname = eq.info.name;
                    eq.info.name = equipname;
                }
                ...
                if(!--count) {
                    setValue("equip_name",$equip.names);
                }
            });
        }
    });
...
if(_query.filter || !settings.equipInventoryIntegration) {
    ...
    GM_addStyle(`
        #eqinv_outer {position:relative; width:1120px; margin:0 0 0 90px}
        ...
        .hvut-in-side {position:absolute; top:44px; left:-80px; width:70px; display:flex; flex-direction:column}
        .hvut-in-side input[type='button'] {margin:3px 0; padding:3px; white-space:normal}
        .hvut-in-side label {color:#000; white-space:nowrap; text-align:left}
    ...
    $element("input", [$element("label",_in.node.side,"Select All"),"afterbegin"] , {type:"checkbox"}, function(){var c=this.checked;_in.list.forEach(eq=>{eq.node.check.checked=c;});});
...
GM_addStyle(`
    ...
    .hvut-se-button {margin:0 30px 10px 10px}
    .hvut-se-remove {visibility:hidden; margin-left:-30px}
    .hvut-se-button:hover + .hvut-se-remove, .hvut-se-remove:hover {visibility:visible}
`);

_se.div = $element("div",_se.form,[".hvut-se-div"]);
$element("input",_se.div,{type:"button",value:"Save current settings",style:"font-weight:bold;margin-bottom:15px"},_se.save);
$element("br",_se.div);
...
_es.salvage_calc = function(eq,d) {
    ...
    eq.data.salvage = s[1]*s[2] + g[1]*g[2] + e[1]*e[2];
};
...
_es.get_item_pane = function(pane,filter,append) {
    ...
    list.forEach(function(eq){
        ...
        if(eq.data.valuable) {
            $element("span",[eq.node.salvage,"afterend"],[" Transfer to Storage",".hvut-es-transfer"],function(){_es.transfer(eq);});
            _es.group["valuable"].item_div.appendChild(eq.node.wrapper);
            if(settings.equipmentShopAutoLock && eq.node.div.dataset.locked==="0") {
                eq.node.div.previousElementSibling.click();
            }
        } else if(append) {
...
_es.get_shop_pane = function(pane,filter,append) {
    ...
    list.forEach(function(eq){
        ...
        if(settings.equipmentShopShowLevel && eq.info.level) {
            eq.node.div.dataset.level = eq.info.level;
        }
        ...
        if(eq.data.salvage > eq.data.value) {
            display = true;
            $element("span",eq.node.sub,[" Salvage "+eq.data.salvage,".hvut-es-salvage2"],function(){_es.shop_salvage(eq);});
        }
...
GM_addStyle(`
    ...
    .hvut-es-select {position:absolute; margin-top:5px; z-index:1}
    .hvut-es-select > input {margin-right:20px}
    ...
    .hvut-es-transfer {font-weight:bold}
    .hvut-es-disabled {text-decoration:line-through}
    .hvut-es-salvage2 {font-weight:bold; color:#090; cursor:pointer}
    div[data-level]::before {content:"[" attr(data-level) "] "}
`);

_es.buttons = $element("div",[$id("eqshop_outer"),"afterbegin"],[".hvut-es-select"]);
$element("input",_es.buttons,{type:"button",value:"Select :",style:"margin-right:0"},function(){_es.item_pane.forEach(function(eq){eq.node.check.checked=(eq.node.div.dataset.locked=="0"&&!eq.data.valuable&&!eq.data.need_salvage);});});
...
$element("input",_es.buttons,{type:"button",value:"Edit Material Price"},function(){$prices.edit("Materials",()=>{if(confirm("You need to refresh the page.")){location.href=location.href;}});});
$element("input",_es.buttons,{type:"button",value:"Edit Protect Filter"},function(){_es.edit_filter("protect");});
...
if(_query.filter || !settings.equipmentShopIntegration) {
    ...
    $element("input",_es.buttons,{type:"button",value:"Hide Equipment",style:"width:125px"},function(){if($id("csp").classList.contains("hvut-hide-on")){$id("csp").classList.remove("hvut-hide-on");this.value="Hide Equipment";}else{$id("csp").classList.add("hvut-hide-on");this.value="Show Equipment";}});
...
GM_addStyle(`
    #itembot_outer {margin:0; left:780px}
    ...
    .hvut-ib-bot {position:absolute; top:50px; left:50px; width:700px; height:600px; margin:0; padding:10px; column-count:3; column-gap:20px; list-style:none; font-size:9pt; line-height:16px; text-align:left; white-space:nowrap}
    .hvut-ib-bot > li {border-width:1px; border-color:#966; border-bottom-style:solid; overflow:hidden; text-overflow:ellipsis; cursor:default}
    .hvut-ib-bot > li:hover {background-color:#fff}
    .hvut-ib-category {margin-top:10px; border-style:solid; font-weight:bold; background-color:#fff; text-align:center}
    .hvut-ib-margin {margin-top:1px; border-top-style:solid}
    .hvut-ib-break {break-after:column}
    .hvut-ib-break + li {margin-top:10px}
    .hvut-ib-bot span {float:left; width:50px; height:16px; margin-right:5px; padding-right:5px; border-right:1px solid; text-align:right}
`);
...
Array.from(_ib.selector.options).forEach(function(o){
    ...
    var bid = _ib.biddata[o.value], // cc, cb, mb
        name = o.text,
    ...
    var li = $element("li",_ib.ul,[`/<span>${bid.cb||""}</span>${name}`],function(){_window.itembot.set_item(o.value);});
...
$qsa("#active_pane .itemlist tr").forEach(function(tr){
    ...
    $element("td",tr,bid.cb);
});

if($qs("#active_pane .itemlist")) {
    $element("tr",[$qs("#active_pane .itemlist"),"afterbegin"],["/<td></td><td>count</td><td>your bid</td>"]);
}
...
GM_addStyle(`
    ...
    .cspp {margin-top:15px}
    #slot_pane {overflow-y:scroll; white-space:nowrap}
    ...
    .hvut-ml-sort > div {margin:0 5px; border:1px solid; box-sizing:border-box; cursor:pointer}
    ...
    .hvut-ml-side input[type='button'] {margin:3px 0; padding:3px; white-space:normal}
    .hvut-ml-side > input:nth-child(5), .hvut-ml-side > input:nth-child(8) {margin-top:17px}
    ...
    .hvut-ml-gains > span {display:inline-block; width:25px; line-height:22px; border-radius:2px; background-color:#630; color:#fff}
    .hvut-ml-gains > ul {display:none; position:absolute; top:2px; right:35px; margin:0; padding:5px 10px; border:1px solid; list-style:none; font-size:9pt; line-height:1.6em; white-space:nowrap; background-color:#fff; z-index:1}
    ...
    .hvut-ml-summary {position:absolute; top:38px; left:10px; max-height:500px; min-width:250px; margin:0; padding:10px; overflow:auto; border:1px solid; list-style:none; background-color:#fff; font-size:10pt; line-height:20px; text-align:left; white-space:nowrap; z-index:3; cursor:default}
    ...
    .hvut-ml-log {position:absolute; top:38px; left:610px; margin:0; padding:10px; width:460px; height:560px; column-count:2; column-gap:10px; border:1px solid; list-style:none; background-color:#fff; font-size:9pt; line-height:16px; text-align:left; white-space:nowrap; z-index:3}
    .hvut-ml-log > li {overflow:hidden; text-overflow:ellipsis}
    .hvut-ml-total {column-span:all; font-weight:bold}
    .hvut-ml-total:nth-child(3) {margin-bottom:16px}
...
_ml.parse = function(mob,html) {
    ...
    mob.wins = parseInt(reg2.test(html) && RegExp.$1);
    reg2.lastIndex = html.indexOf("Killing Blows");
    mob.kills = parseInt(reg2.test(html) && RegExp.$1);

    var pa = Array.from(html.matchAll(/<div>\+(\d+)<\/div>/g)).map(m=>parseInt(m[1])),
        er = pa.splice(6,6);
...
make_summary : function() {
    ...
    $element("li",_ml.main.node.summary,mobs.length+" monster(s), "+gains+" gift(s), "+_ml.price2str(profits)+" credits");
...
make_log : function(mob) {
    ...
    mob.node.log.prepend(
        $element("li",null,[".hvut-ml-total"," For "+Math.floor(round/24)+" days "+(round%24)+" hours / Since "+(new Date(date)).toLocaleString()]),
        $element("li",null,[".hvut-ml-total"," - Total: "+count+" gifts, "+_ml.price2str(sum)+" credits"]),
        $element("li",null,[".hvut-ml-total"," - Daily: "+(Math.round(count/daily*24*10)/10)+" gifts, "+_ml.price2str(sum/daily*24)+" credits"])
    );
},
reset_log : function() {
...
_ml.main.node.sort = {};
_ml.main.node.sort.div = $element("div",[$id("slot_pane"),"beforebegin"],[".hvut-ml-sort"]);
...
$element("input",_ml.main.node.side,{type:"button",value:"Gift Log"},function(){_ml.main.toggle_log(_ml.mobs.sum);});
$element("input",_ml.main.node.side,{type:"button",value:"Edit Material Price"},function(){$prices.edit("Materials",_ml.main.change_price);});
...
$element("input",_ml.main.node.side,{type:"button",value:"Monster Upgrader",id:"hvut-ml-fd-open"},function(){_ml.upgrade.toggle();});
$element("input",_ml.main.node.side,{type:"button",value:"Power Level Calculator"},function(){_ml.lab.open();});
...
init : function() {
    if(!_ml.inventory) {
        _ml.upgrade.node.open = $id("hvut-ml-fd-open");
        _ml.upgrade.node.open.disabled = true;
        _ml.inventory = {};
        $ajax.add("GET","?s=Character&ss=it",null,function(r){
            Array.from( substring(r.responseText,'<table class="nosel itemlist">',["</table>",true],true).firstElementChild.rows ).forEach(function(tr){
            ...
            _ml.upgrade.node.open.disabled = false;
            _ml.upgrade.init();
        });
        return;
    }
    ...
    GM_addStyle(`
        ...
        .hvut-ml-fd-btn {float:right; width:250px; display:flex; flex-wrap:wrap; justify-content:space-between}
        .hvut-ml-fd-btn > input {width:32%; margin-bottom:5px}
    `);
    ...
    _ml.upgrade.node.class = $element("span",span,[".hvut-ml-fd-class"]).appendChild( $element("select",null,{style:"width:65px;margin:0"},{change:_ml.upgrade.filter}) );

    $element("option",_ml.upgrade.node.class,{text:"All",value:""});
    ["Arthropod","Avion","Beast","Celestial","Daimon","Dragonkin","Elemental","Giant","Humanoid","Mechanoid","Reptilian","Sprite","Undead"].forEach(function(c){
        $element("option",_ml.upgrade.node.class,{text:c,value:c});
    });
    ...
    _ml.upgrade.node.run = $element("input",btn_pane,{type:"button",value:"Run"},_ml.upgrade.run);
    ...
    _ml.mobs.forEach(function(mob){
        ...
        $element("span",span,[" "+mob.class,".hvut-ml-fd-class"]);
        ...
        if(mob.need_update) {
            ...
            $ajax.add("GET","?s=Bazaar&ss=ml&slot="+mob.index,null,
                function(r){
                    ...
                    _ml.upgrade.node.open.value = "Monster Update... ("+count+"/"+total+")";
                    ...
                    if(total === count) {
                        setValue("ml_log",_ml.log);
                        _ml.upgrade.node.open.disabled = false;
                        _ml.upgrade.node.open.value = "Monster Upgrader";
                        if(status === 1) {
...
sum : function(skip) {
    ...
    _ml.upgrade.pa.forEach(function(e){
        var li = e.li;
        li.innerHTML = "";
        $element("span",li,e.crystal.slice(11));
    ...
    _ml.upgrade.node.run.disabled = !stock;
},

filter : function() {
    var mclass = _ml.upgrade.node.class.value,
    ...
    var filtered = 0;
    _ml.mobs.forEach(function(mob){
        if( (!mclass||mclass===mob.class) && (!pl||pl===mob.pl) && (!pl_min||pl_min<=mob.pl) && (!pl_max||pl_max>=mob.pl) ) {
            mob.visible = true;
...
init : function() {
    ...
    GM_addStyle(`
        #hvut-ml-lab-pane {display:flex; position:absolute; top:27px; left:0; width:100%; height:675px; justify-content:center; align-items:center; z-index:9; background-color:#EDEBDF; font-size:10pt; text-align:left; color:#000; white-space:nowrap}
        #hvut-ml-lab-pane input[type='button'] {padding:0}

        .hvut-ml-lab-right {height:625px; margin-left:20px}
        .hvut-ml-lab-menu {display:flex; flex-wrap:wrap; justify-content:space-between; width:250px}
        .hvut-ml-lab-menu > input {margin-bottom:4px}
        .hvut-ml-lab-table {margin-top:20px; width:480px; border-collapse:collapse}
        .hvut-ml-lab-table tr:first-child {font-weight:bold}
        .hvut-ml-lab-table td {border:1px solid #999; padding:2px 5px}
        .hvut-ml-lab-table td:first-child {text-align:right}

        .hvut-ml-lab-left {width:600px; height:515px; margin-top:110px; overflow:auto; line-height:25px}
        .hvut-ml-lab-left > div {display:flex; width:572px; margin:5px 0; padding:5px 0; border:1px solid #999}
        .hvut-ml-lab-left > div:first-child {position:absolute; margin-top:-110px; background-color:#fff}
        .hvut-ml-lab-left > div > div {width:240px; padding:5px; border-left:1px solid #999}
        .hvut-ml-lab-left > div > div:first-child {width:60px; border-left:none}

        .hvut-ml-lab-del {width:25px; margin-right:10px}
        .hvut-ml-lab-cnt {width:45px; text-align:right}

        .hvut-ml-lab-lv {display:inline-block; width:140px; text-align:center}
        .hvut-ml-lab-lv > span {display:inline-block; width:18px; line-height:18px; border:1px solid; margin:0 1px; text-align:center; background-color:#ddd; color:#666; border-radius:3px; cursor:default}
        .hvut-ml-lab-up {background-color:#fff !important; color:#000 !important}
        .hvut-ml-lab-btn {display:inline-block; width:140px; text-align:center}
        .hvut-ml-lab-btn > input {width:25px; margin:0 5px}
        .hvut-ml-lab-text {display:inline-block; width:140px; text-align:right}
    ...
    node.pane = $element("div",null,["#hvut-ml-lab-pane"]);

    node.left = $element("div",node.pane,[".hvut-ml-lab-left"]);

    var total = $element("div",node.left);
    $element("div",total).append(
        $element("span",null,"Monsters"),$element("br"),$element("br"),
        node.count = $element("input",null,{type:"number",className:"hvut-ml-lab-cnt",min:0,max:200,readOnly:true})
    );
    $element("div",total).append(
        $element("span",null,"Primary Attributes"),$element("br"),
        $element("span",null,[" Crystal",".hvut-ml-lab-text"]),
        node.pa_total = $element("span",null,[".hvut-ml-lab-crystal"]),$element("br"),
        $element("span",null,[" Difference",".hvut-ml-lab-text"]),
    ...
    node.right = $element("div",node.pane,[".hvut-ml-lab-right"]);

    var menu = $element("div",node.right,[".hvut-ml-lab-menu"]);
    $element("input",menu,{type:"button",value:"Save",style:"width:32%"},function(){_ml.lab.save();});
    $element("input",menu,{type:"button",value:"Load",style:"width:32%"},function(){_ml.lab.load();});
    $element("input",menu,{type:"button",value:"Close",style:"width:32%"},function(){_ml.lab.close();});
    $element("input",menu,{type:"button",value:"Add Monster",style:"width:100%;margin-top:16px"},function(){_ml.lab.add();});
    Object.keys(_ml.lab.preset).forEach(e=>{
        $element("input",menu,{type:"button",value:e,style:"width:24%"},function(){_ml.lab.add(_ml.lab.preset[e]);});
    });
...
open : function() {
    _ml.lab.init();
    $id("mainpane").appendChild(_ml.lab.node.pane);
},
close : function() {
    _ml.lab.node.pane.remove();
},
remove : function(m) {
    if(m) {
        m.node.div.remove();
...
add : function(j) {
    ...
    $element("br",sub);
    m.node.count = $element("input",sub,{type:"number",className:"hvut-ml-lab-cnt",value:m.json.count,min:0,max:200},{input:function(){_ml.lab.change(m,"count");},focus:_ml.lab.select});

    sub = $element("div",m.node.div);
    $element("span",sub,"Primary Attributes");
    $element("br",sub);

    span = $element("span",sub,[".hvut-ml-lab-lv"]);
    m.node.pa = [];
    for(let i=0;i<6;i++) {
...
change : function(m,p,v) {
    ...
    if(m.node.count.validity.valid) {
        ...
        m.pa_avg = (data.pa_sum[pa_lv] * (6-pa_up) + data.pa_sum[pa_lv+1] * (pa_up)) / 6;
        m.er_avg = (data.er_sum[er_lv] * (6-er_up) + data.er_sum[er_lv+1] * (er_up)) / 6;
...
_ss.node = {};
_ss.equip = {capacity:0,current:0,request:0,received:0,sold:0,salvaged:0,total:0};
_ss.items = {};
_ss.trophies = {
...
_ss.offer = function(name,count) {
    ...
    if(item.type === "Trophy") {
        ...
        if(select_img) {
            select_reward = select_img.id.slice(7);
        } else {
    ...
    if(!_ss.node.results) {
        _ss.node.results = $element("div",$id("shrine_outer"),[".hvut-ss-results"]);
    }
    if(!item.results) {
        ...
        for(let r in item.results) {
            item.node.ul.appendChild(item.results[r].li).classList.add("hvut-none");
        }
    ...
    while(count--) {
        $ajax.add("POST","?s=Bazaar&ss=ss","select_item="+item.id+"&select_reward="+select_reward,_ss.response,null,{name:name});
    }
};

_ss.response = function(r) {
    var name = r.context.name,
        item = _ss.items[name],
    ...
    Array.from( substring(r.responseText,'<div id="messagebox"','<div id="mainpane">',true).firstElementChild.lastElementChild.children ).forEach(function(e){
        ...
        } else if(e.includes("Sold it for")) {
            _ss.equip.sold++;
        } else if(e.includes("Salvaged it for")) {
            _ss.equip.salvaged++;
    ...
    rewards.forEach(function(n){
        ...
        for(let r in results) {
            results[r].sp.textContent = (results[r].count*100/item.recieved).toFixed(1)+" %";
            results[r].sc.textContent = " ["+results[r].count+"] ";
        }

        if(item.type === "Trophy") {
            if(settings.shrineTrackEquip.includes(r)) {
...
_ss.view_log = function() {
    ...
    for(let i in _ss.log) {
        let keys = Object.keys(_ss.log[i]),
        ...
        for(let r in _ss.log[i]) {
            let count = _ss.log[i][r];
            total += count;
            if(!list[r]) {
        ...
        $element("h4",div,[" "+i+" ("+total+")",".hvut-ss-h4"]);
    ...
    $element("input", $element("div",div,["!margin-top:20px;padding:10px;border-top:1px solid"]), {type:"button",value:"Reset Log"},function(){if(confirm("Shrine reward log will be deleted.\n\nContinue?")){deleteValue("ss_log");location.href=location.href;}});
...
_ss.calc_value = function(p) {
    var text = [],
        value = 0;
    for(let name in _ss.items) {
        let item = _ss.items[name];
        if(item.value) {
            let offer = item.stock - item.stock%item.bulk;
            value += offer * item.value;
            text.push(offer.toLocaleString()+" x "+name+" @ "+item.value.toLocaleString()+" = "+(offer*item.value).toLocaleString());
        }
    }
    var sum = "You have "+value.toLocaleString()+" credits worth of trophies in the inventory.";
    _ss.node.trophy.value = sum;
...
GM_addStyle(`
    ...
    #inv_item .itemlist td:nth-child(1) {width:175px !important}
    #inv_item .itemlist td:nth-child(2) {width:60px}
    #inv_item .itemlist td:nth-child(3) {width:30px; padding-left:5px; text-align:left; font-size:smaller; color:#966}
    #inv_item .itemlist td:nth-child(4) {width:50px}
    #inv_item .itemlist td:nth-child(5) {width:140px; text-align:left}
...
_ss.node.left_buttons = $element("div",[$id("inv_item").parentNode,"afterbegin"],[".hvut-ss-buttons"]);
$element("input",_ss.node.left_buttons,{type:"button",value:"Log"},_ss.view_log);
$id("csp").classList.add("hvut-hide-on");
$element("input",_ss.node.left_buttons,{type:"button",value:"Show Items",style:"width:90px"},function(){if($id("csp").classList.contains("hvut-hide-on")){$id("csp").classList.remove("hvut-hide-on");this.value="Hide Items";}else{$id("csp").classList.add("hvut-hide-on");this.value="Show Items";}});

_ss.node.right_buttons = $element("div",[$id("shrine_info").parentNode,"afterbegin"],[".hvut-ss-buttons"]);
$element("input",_ss.node.right_buttons,{type:"button",value:"Results"},function(){if(_ss.node.results){_ss.node.results.classList.toggle("hvut-none");}});
_ss.node.equip = $element("input",_ss.node.right_buttons,{type:"button",value:"Equip Slots",style:"min-width:250px"});
_ss.node.trophy = $element("input",[$id("shrine_trophy"),6],{type:"button",value:"Calculate Trophy Value",style:"margin:10px 0"},function(){_ss.calc_value(true);});
...
_mm.send = function(queue) {
    ...
    if(queue) {
        ...
        var deduction = queue.deduction || 0,
            error;
        s.batch.forEach(function(b){
            if(deduction) {
                b.deduction = Math.min(b.cod,deduction);
                deduction -= b.deduction;
            }
            var cod = b.cod - b.deduction;
            if(cod && cod < 10) {
                error = true;
            }
        });
        if(error) {
            alert("CoD must be at least 10 credits.");
            s.batch = null;
            return;
        }
    ...
    if(!b.status) { // attach
        ...
        if(b.cod > b.deduction) { // cod
            b.status = 3;
            _mm.log("#"+(s.index+1)+": Setting CoD");
            $ajax.add("POST","?s=Bazaar&ss=mm&filter=new","mmtoken="+_mm.mmtoken+"&action=attach_cod&action_value="+(b.cod-b.deduction),function(r){
                if(_mm.check(r.responseText)) {
                    return;
                }
                b.status = 4;
        ...
        if(b.atext) {
            ...
            if(b.cod) {
                ...
                if(b.deduction) {
                    ...
                    message_body += "\nCoD: "+(b.cod-b.deduction).toLocaleString()+" Credits";
                    if(b.cod < b.deduction) {
                        message_body += "\n=> CoD: 0 Credit";
                    }
                }
            }
...
GM_addStyle(`
    ...
    .hvut-mm-right-menu::after {content:''; display:block; clear:both}
    .hvut-mm-right-menu > input {margin:0 10px; padding:2px 10px; font-weight:bold}

    .hvut-mm-field {margin:0; padding:0; border:none; height:490px; overflow:hidden scroll}
    ...
    #hvut-mm-right input {box-sizing:border-box}
    #hvut-mm-right .hvut-mm-sub > input {font-weight:normal}
    #hvut-mm-right .hvut-mm-check {width:auto; margin-right:5px}
    ...
    #hvut-mm-credits div {margin:20px 0}
    #hvut-mm-credits span {display:inline-block; margin-right:10px; font-weight:bold; text-align:right}
`);
...
$element("span",_mm.node.left,{textContent:"To:",style:"clear:left;float:left;margin:2px;padding:2px;width:60px;font-size:10pt;line-height:17px;text-align:right"});
_mm.node.to_name = $element("input",_mm.node.left,{type:"text",value:_mm.form.message_to_name.value||"",tabIndex:1,style:"float:left;margin:2px;padding:2px;width:370px;font-size:10pt;line-height:17px;font-weight:bold"});
$element("input",_mm.node.left,{type:"button",value:"Edit List",style:"float:left;margin:2px;height:23px;width:76px"},_mm.userlist.popup);
$element("span",_mm.node.left,{textContent:"Subject:",style:"clear:left;float:left;margin:2px;padding:2px;width:60px;font-size:10pt;line-height:17px;text-align:right"});
_mm.node.subject = $element("input",_mm.node.left,{type:"text",value:_mm.form.message_subject.value||"",tabIndex:2,style:"float:left;margin:2px;padding:2px;width:450px;font-size:10pt;line-height:17px;font-weight:bold"});
...
$element("input",_mm.node.right_menu,{type:"button",value:"Use Default MoogleMail",style:"float:right"},function(){location.href=location.href+"&hvut=disabled";});
...
_mm.node.item_search = $element("input",_mm.node.item_menu,{placeholder:"Search",style:"width:190px"},{input:function(e){if(e.keyCode===27){_mm.search("item","",true);}else{_mm.search("item",this.value);}}});
...
$element("input",_mm.node.item_menu,{placeholder:"Count",style:"width:60px;text-align:right"},{input:function(){_mm.item_count(this.value);}});
...
_mm.node.deduction = $element("input",_mm.node.item_menu,{placeholder:"CoD Deduction",pattern:"(\\d+|\\d{1,3}(,\\d{3})*)(\\.\\d+)?[KMkm]?",style:"width:100px;margin-left:auto;text-align:right"},{input:_mm.calc});
...
_mm.node.credits_count = $element("input",_mm.node.credits_sub,{className:"hvut-mm-count",placeholder:"count",pattern:"(\\d+|\\d{1,3}(,\\d{3})*)(\\.\\d+)?[KMkm]?",style:"width:80px"});
...
_mm.node.hath_count = $element("input",_mm.node.hath_sub,{className:"hvut-mm-count",placeholder:"count",pattern:"\\d+|\\d{1,3}(,\\d{3})*",style:"width:80px"});
_mm.node.hath_price = $element("input",_mm.node.hath_sub,{className:"hvut-mm-price",placeholder:"price",pattern:"(\\d+|\\d{1,3}(,\\d{3})*)(\\.\\d+)?[KMkm]?",style:"width:60px"});
...
_mm.multi_send = function() {
    ...
    _mm.node.multi_text.value.split("\n").forEach(function(t,i){
        ...
        if(!a || !(a=a.trim())) {
        } else if(/^([0-9,.]+)([ckmh])?$/.exec(a.toLowerCase())) {
            var c = RegExp.$1,
                u = RegExp.$2 || "c";
            if(u === "h") {
                m.mail.pane = "hath";
...
_mm.load_mail = function(mid,view,post,callback) {
    ...
    if(view) {
        ...
        _mm.close_mail();
        _mm.current = mid;
        $element("div",_mm.view_mail.div,[" Loading...",".hvut-mm-loading"]);
    ...
    $ajax.add("POST","?s=Bazaar&ss=mm&mid="+mid,post,function(r){
        ...
        if(html.includes('<div id="mmail_outer">')) {
            ...
            var mail = data.mail = {
                ...
                return : $qs("#mmail_showbuttons > img[src*='returnmail.png']",content) ? true : false,
                recall : $qs("#mmail_showbuttons > img[src*='recallmail.png']",content) ? true : false,
                reply : $qs("#mmail_showbuttons > img[src*='reply.png']",content) ? true : false,
                take : $qs("#mmail_attachremove > img[src*='attach_takeall.png']",content) ? true : false
            };
    ...
    if(!_mm.uid) {
        ...
        equips.forEach(function(eq){
            ...
            $ajax.add("POST","?s=Forge&ss=fo&filter="+({"One-handed Weapon":"1handed","Two-handed Weapon":"2handed","Staff":"staff","Shield":"shield","Cloth Armor":"acloth","Light Armor":"alight","Heavy Armor":"aheavy"})[eq.info.category],"select_item="+eq.info.eid,function(r){
                var html = r.responseText;
                if(html.includes('<div id="messagebox"')) {
                    status.error = substring(html,'<div id="messagebox"','<div id="mainpane">',true).firstElementChild.lastElementChild.textContent.trim();
...
GM_addStyle(`
    ...
    #hvut-mm-view {position:absolute; top:81px; right:11px; display:flex; flex-direction:column; width:613px; height:566px; padding:5px; border:2px solid; background-color:#fff; z-index:2; color:#000; font-size:10pt; text-align:left}
    .hvut-mm-failed {background-color:#eee !important}
    #hvut-mm-view dl {list-style:none; margin:5px; padding:0}
    ...
    #hvut-mm-view dd:nth-of-type(2n+1) {width:340px}
    #hvut-mm-view dd:nth-of-type(2n) {white-space:nowrap}
    .hvut-mm-rts::before {content:'[MoogleMail]'; color:#999; margin-right:5px}
    #hvut-mm-view textarea {flex-basis:180px; margin:5px; padding:10px; font-size:10pt}
    .hvut-mm-btns {display:flex; margin:5px}
    ...
    #hvut-mm-view li:first-child {margin-top:0; padding:0 0 0 5px; border:1px solid; background-color:#eee; font-weight:bold}
    ...
    #hvut-mm-view li > input {margin-left:5px; padding:0 3px; border:1px solid; line-height:18px; text-align:right; background-color:transparent}
...
_mm.node.search_filter = $element("select",_mm.node.search_div,{innerHTML:"<option value='all'>All</option><option value='inbox'>Inbox</option><option value='read'>Read</option><option value='sent'>Sent</option>",style:"width:70px;margin:0"});
...
_mm.node.log = $element("textarea",$element("div",$id("mmail_outer"),[".hvut-none","!position:absolute;top:81px;right:638px;border:2px solid;background-color:#EDEBDF"]),{readOnly:true,spellcheck:false,style:"width:300px;height:300px;margin:10px;font-size:9pt"});
...
if(settings.showLottery && $qs("img[src$='lottery_next_d.png']")) {
    ...
    $element("input",_lt.label,{type:"checkbox",checked:!_lt.json.hide},{change:function(){_lt.json.hide=!this.checked;setValue(_query.ss+"_show",_lt.json);}});
...
GM_addStyle(`
    ...
    #arena_list > tbody > tr > th:nth-child(1) {width:300px}
    ...
    #arena_list > tbody > tr > th:nth-child(8) > input {width:90px; padding:0}
    #arena_list > tbody > tr > td {height:24px}
    #arena_list > tbody > tr > td > div {width:100% !important; top:3px; left:0}

    #mainpane {padding-left:8px; padding-right:8px}
    #mainpane.hvut-ar-details {padding-left:8px; padding-right:8px}
    #mainpane.hvut-ar-details #arena_list tr> * {display:table-cell}
    #mainpane.hvut-ar-details #arena_list > tbody > tr > th:nth-child(1) {width:482px}
    #mainpane.hvut-ar-details .hvut-bt-div {display:none}
`);

_ar.toggle_details = function() {
    var mainpane = $id("mainpane");
    mainpane.classList.toggle("hvut-ar-details");
    _ar.detail_button.value = mainpane.classList.contains("hvut-ar-details") ? "Hide Details" : "Show Details";
};
...
GM_addStyle(`
    #arena_list tr>*:nth-child(2), #arena_list tr>*:nth-child(5), #arena_list tr>*:nth-child(6), #arena_list tr>*:nth-child(7) {display:none}
`);

_ar.split_colspan($id("arena_list"));
_ar.detail_button = $element("input",$id("arena_list").rows[0].cells[7],{type:"button",value:"Show Details"},_ar.toggle_details);
...
GM_addStyle(`
    #arena_list tr>*:nth-child(2), #arena_list tr>*:nth-child(4), #arena_list tr>*:nth-child(5), #arena_list tr>*:nth-child(7) {display:none}
`);
...
_iw.calc_potency = function(eq,round) {
    ...
    $element("span",eq.node.calc,[" +"+exp+" ("+round+") =",".hvut-iw-gain"],function(){_iw.calc_potency(eq,prompt("Enter the number of Rounds"));});
...
_iw.show_potency = function(eq) {
    if(!eq.info.round) {
        eq.info.pxp = $equip.parse.pxp(eq.info.pxp2,eq.info.tier);
...
GM_addStyle(`
    #filterbar {position:absolute; left:8px}
    #itemworld_outer {width:auto; margin-top:40px}
...
if( (_iw.latest=$id("e"+_iw.json[_iw.filter])) ) {
    ...
    _iw.latest.parentNode.style.cssText = "background-color:#fff;border:1px solid";
    _iw.latest.click();
}
...
$battle.view = function(eq) {
    ...
    if(eq.data.repair) {
        for(let r in eq.data.repair) {
            $element("li",$battle.node.repair,r+" x "+eq.data.repair[r]);
        }
    } else {
    ...
    for(let name in $battle.enchant_data) {
        ...
        if(item[cat]) {
            let li = $element("li",$battle.node.enchant),
                count = parseInt((cat==="weapon" && name.includes("Infusion of ")) ? settings.equipEnchantWeapon : settings.equipEnchantArmor) || 1,
            ...
            $element("span",li,[" "+item.effect,".hvut-bt-shard"],function(){$battle.enchant(eq,name,1);});
...
$battle.load_dynjs = function(url) {
    $ajax.add("GET",url,null,function(r){
        var dynjs_loaded = JSON.parse(substring(r.responseText,"{",["",-1]));
...
$battle.display_inventory = function() {
        ...
        if($battle.repairall) {
            for(let e in $battle.repairall) {
                if($battle.repairall[e]) {
                    ...
                    $element("li",$battle.node.repairall,[" "+e+" x "+$battle.repairall[e]+" ("+c+")",c<$battle.repairall[e]?".hvut-bt-warn":""]);
                }
            }
        ...
        $battle.node.inventory.innerHTML = "";
        settings.equipEnchantInventory.forEach(function(e){
            var c = $battle.inventory[e] || 0;
            $element("li",$battle.node.inventory,[" "+e+" ("+c+")",c<settings.equipEnchantInventoryCheck?".hvut-bt-warn":""]);
        });
...
GM_addStyle(`
    .hvut-bt-div {position:absolute; bottom:8px; width:599px; color:#000; font-size:10pt; line-height:20px; white-space:nowrap}
    ...
    .hvut-bt-div > ul:before {content:attr(header); position:absolute; top:0; width:100%; border-bottom:1px solid; line-height:20px; background-color:#edb; font-weight:bold}
    ...
    .hvut-bt-enc > span {display:inline-block; margin:0 3px; color:#f00}
    .hvut-bt-cdt {position:absolute; top:0; right:0; width:59px; height:100%; border-left:1px solid #000; font-size:9pt; white-space:normal}
    ...
    .hvut-bt-imbue {color:#06f; cursor:pointer}
    .hvut-bt-shard {flex-grow:1; overflow:hidden; text-overflow:ellipsis; text-align:left; color:#00c; cursor:pointer}
    .hvut-bt-stock {}
...
if(settings.equipEnchantPosition === "right") {
    GM_addStyle(`
        #mainpane {padding-right:618px}
        #popup_box {left:636px !important}
        .hvut-bt-div {right:8px}
    `);

} else {
    GM_addStyle(`
        #mainpane {padding-left:618px}
        #popup_box {left:232px !important}
        .hvut-bt-div {left:8px}
    `);
}

$battle.node = {};
$battle.node.div = $element("div",$id("mainpane"),[".hvut-bt-div"]);
$battle.node.equip = $element("ul",$battle.node.div,{className:"hvut-bt-equip",header:"EQUIPMENT"});
$battle.node.enchant = $element("ul",$battle.node.div,{className:"hvut-bt-enchant",header:"ENCHANT"});
$battle.node.repair = $element("ul",$battle.node.div,{className:"hvut-bt-repair",header:"REPAIR"},function(){$battle.repair($battle.current);});
$battle.node.repairall = $element("ul",$battle.node.div,{className:"hvut-bt-repairall",header:"REPAIR ALL"},function(){$battle.repair("all");});
$battle.node.inventory = $element("ul",$battle.node.div,{className:"hvut-bt-inventory",header:"ITEM INVENTORY"});
...
_up.upgrade_costs = [];
_up.upgrade_costs_damage = [];

_up.forge_exp = {"Low-Grade":1,"Mid-Grade":5,"High-Grade":20,"Wispy Catalyst":3,"Diluted Catalyst":13,"Regular Catalyst":25,"Robust Catalyst":63,"Vibrant Catalyst":125,"Coruscating Catalyst":250};

_up.bindings = {
"Physical Damage":"Binding of Slaughter",
"Physical Hit Chance":"Binding of Balance",
"Physical Crit Chance":"Binding of Isaac",
"Magical Damage":"Binding of Destruction",
"Magical Hit Chance":"Binding of Focus",
"Magical Crit Chance":"Binding of Friendship",
"Physical Defense":"Binding of Protection",
"Magical Defense":"Binding of Warding",
"Evade Chance":"Binding of the Fleet",
"Block Chance":"Binding of the Barrier",
"Parry Chance":"Binding of the Nimble",
"Resist Chance":"Binding of Negation",
"Elemental Proficiency":"Binding of the Elementalist",
"Divine Proficiency":"Binding of the Heaven-sent",
"Forbidden Proficiency":"Binding of the Demon-fiend",
"Deprecating Proficiency":"Binding of the Curse-weaver",
"Supportive Proficiency":"Binding of the Earth-walker",
"Fire Spell Damage":"Binding of Surtr",
"Cold Spell Damage":"Binding of Niflheim",
"Elec Spell Damage":"Binding of Mjolnir",
"Wind Spell Damage":"Binding of Freyr",
"Holy Spell Damage":"Binding of Heimdall",
"Dark Spell Damage":"Binding of Fenrir",
"Crushing Mitigation":"Binding of Dampening",
"Slashing Mitigation":"Binding of Stoneskin",
"Piercing Mitigation":"Binding of Deflection",
"Fire Mitigation":"Binding of the Fire-eater",
"Cold Mitigation":"Binding of the Frost-born",
"Elec Mitigation":"Binding of the Thunder-child",
"Wind Mitigation":"Binding of the Wind-waker",
"Holy Mitigation":"Binding of the Thrice-blessed",
"Dark Mitigation":"Binding of the Spirit-ward",
"Strength Bonus":"Binding of the Ox",
"Dexterity Bonus":"Binding of the Raccoon",
"Agility Bonus":"Binding of the Cheetah",
"Endurance Bonus":"Binding of the Turtle",
"Intelligence Bonus":"Binding of the Fox",
"Wisdom Bonus":"Binding of the Owl",
};

_up.node = {};

_up.init_costs = function() {
    _up.upgrade_costs_damage.length = 0;
    _up.upgrade_costs.length = 0;

    var list = new Array(150);
    list.fill([6,0,0],0);
    list.fill([5,1,0],5);
    list.fill([4,2,0],12);
    list.fill([3,3,0],20);
    ...
    list.fill([0,1,5],85);
    list.fill([0,0,6],92);
    list = list.slice(({"Superior":0,"Exquisite":15,"Magnificent":30,"Legendary":50})[_up.quality]||0);
    list = list.slice(0,100);
    var catalysts = ["Wispy Catalyst","Diluted Catalyst","Regular Catalyst","Robust Catalyst","Vibrant Catalyst","Coruscating Catalyst"];
    catalysts = catalysts.slice(({"Superior":0,"Exquisite":1,"Magnificent":2,"Legendary":3})[_up.quality]||0);
    catalysts = catalysts.slice(0,3);

    list.forEach(function(e,i){
        var obj = {costs:{}};
        if(e[0]) {obj.costs["Low-Grade "+_up.type]=e[0];}
        if(e[1]) {obj.costs["Mid-Grade "+_up.type]=e[1];}
        if(e[2]) {obj.costs["High-Grade "+_up.type]=e[2];}
        if(i>4 && !_isekai) {
            obj.binding = true;
        }
        obj.forge_exp = _up.forge_exp["Low-Grade"]*e[0] + _up.forge_exp["Mid-Grade"]*e[1] + _up.forge_exp["High-Grade"]*e[2];

        if(i<5 || i<95 && i%2===0) {
            var obj_ = JSON.parse(JSON.stringify(obj)),
                i_ = i<5?i : ((i-4)/2)+4,
                c_ = catalysts[i_<13?0:i_<25?1:2];
            obj_.costs[c_] = 1;
            obj_.forge_exp += _up.forge_exp[c_];
            obj_.gear_exp = Math.ceil(obj_.forge_exp/10);
            _up.upgrade_costs.push(obj_);
        }

        var c = catalysts[i<25?0:i<50?1:2];
        obj.costs[c] = 1;
        obj.forge_exp += _up.forge_exp[c];
        obj.gear_exp = Math.ceil(obj.forge_exp/10);
        _up.upgrade_costs_damage.push(obj);
    });
};

_up.sort_costs = function(object) {
    var keys = Object.keys(object),
        index = ["Wispy Catalyst","Diluted Catalyst","Regular Catalyst","Robust Catalyst","Vibrant Catalyst","Coruscating Catalyst","Low-Grade","Mid-Grade","High-Grade","Crystallized Phazon","Shade Fragment","Repurposed Actuator","Defense Matrix Modulator","Binding of"];
    keys.sort((a,b)=>index.findIndex(e=>a.includes(e))-index.findIndex(e=>b.includes(e)));
...
_up.calc = function(data) {
    data.costs = {};
    data.forge_exp = 0;
    data.gear_exp = 0;
    data.upgrade_costs.slice(data.level,data.to).forEach(function(u){
        for(let item in u.costs) {
            if(!data.costs[item]) {
                data.costs[item] = 0;
            }
            data.costs[item] += u.costs[item];
        }
        if(u.binding) {
            if(!data.costs[data.binding]) {
                data.costs[data.binding] = 0;
            }
            data.costs[data.binding]++;
        }
        data.forge_exp += u.forge_exp;
        data.gear_exp += u.gear_exp;
    });
    if(_up.rare && data.to>_up.max) {
        data.costs[_up.rare] = data.to - _up.max;
    }
    _up.sort_costs(data.costs);
};

_up.calc_list = function(list) {
    var costs = {},
        max = 0,
        forge_exp = 0,
        gear_exp = 0;
    list.forEach(function(data){
        ...
        for(let item in data.costs) {
            if(!costs[item]) {
                costs[item] = 0;
            }
            costs[item] += data.costs[item];
        }
        if(max < data.to) {
            max = data.to;
        }
    ...
    if(_up.rare && max>_up.max) {
        costs[_up.rare] = max - _up.max;
    }
    _up.sort_costs(costs);

    var cost = 0,
        prices = $prices.get("Materials");
    for(let item in costs) {
        cost += costs[item] * (prices[item]||0);
    }

    return {costs,cost,forge_exp,gear_exp};
};


if($id("equip_extended")) {

_up.calc_upgrade = function(data,bg) {
    if(data) {
        data.node.table.innerHTML = "";

        if(data.node.input.validity.valid) {
            data.to = parseInt(data.node.input.value);
            _up.calc(data);

            for(let item in data.costs) {
                let count = data.costs[item],
                    stock = _up.inventory[item] || 0;
                $element("tr",data.node.table,[`/<td>${item}</td><td>${count}</td><td>(${stock})</td>`,count>stock?".hvut-up-nostock":""]);
            }
            data.valid = true;
    ...
    var {costs,cost,forge_exp,gear_exp} = _up.calc_list(_up.list);
    _up.costs = costs;
    ...
    $element("li",_up.node.summary,["/<span>Forge EXP</span>+"+forge_exp.toLocaleString()]);
    $element("li",_up.node.summary,["/<span>Gear EXP</span>+"+gear_exp.toLocaleString()]);
    $element("li",_up.node.summary,["/<span>Potency Tier</span>"+tier+(tier<10?" ("+pxp1+ " / "+pxp2+")":" (MAX)")]);
    $element("li",_up.node.summary,["/<span>Total Cost</span>"+cost.toLocaleString()]);

    _up.node.costs.innerHTML = "";
    for(let item in _up.costs) {
        let count = _up.costs[item],
            stock = _up.inventory[item] || 0;
        $element("tr",_up.node.costs,[`/<td>${item}</td><td>${count}</td><td>(${stock})</td>`,count>stock?".hvut-up-nostock":""]);
    }
    ...
    } else if(_up.list.some(data=>data.to>data.forge_max)) {
...
_up.upgrade = function() {
    if(!_up.calc_upgrade() || !confirm("Are you sure you wish to upgrade this equipment?")) {
        return;
    }
    _up.node.upgrade.disabled = true;
...
_up.buy_catalysts = function() {
    var list = {},
        buy_count = 0;
    Object.keys(_up.costs).filter(e=>e.includes("Catalyst")).forEach(function(item){
        var count = _up.costs[item] - (_up.inventory[item]||0);
        if(count > 0) {
            list[item] = {count};
            buy_count++;
        }
...
_up.load_inventory = function() {
    ...
    $ajax.add("GET","?s=Character&ss=it",null,function(r){
        ...
        _up.node.catalysts.disabled = false;
        _up.list.forEach(e=>_up.calc_upgrade(e,true));
        _up.calc_upgrade();
    });
};

GM_addStyle(`
    #forge_outer {width:1110px}
    #forge_outer + div {width:380px !important; margin:0 0 0 59px !important}
    #leftpane {width:380px !important; margin-left:0 !important}
    #leftpane > div:not(#equip_extended), #leftpane > div:not(#equip_extended) div {width:auto !important}
    #rightpane {width:380px !important}
    #rightpane > div {width:auto !important}
    .hvut-up-div {float:left; width:330px; height:400px; margin:200px 0 0 20px}
    ...
    .hvut-up-summary {margin:10px 0 0; padding:10px; width:310px; list-style:none; font-size:10pt; text-align:right}
    .hvut-up-summary span {float:left; width:200px}
    .hvut-up-costs {margin:0; padding:10px; width:310px; table-layout:fixed; border-spacing:0; font-size:10pt; text-align:right; white-space:nowrap}
    .hvut-up-costs td {padding:0; overflow:hidden; text-overflow:ellipsis}
    .hvut-up-costs td:nth-child(1) {width:200px}
...
$element("input",_up.node.div,{type:"button",value:"Edit Price",tabIndex:-1,style:"width:80px"},function(){$prices.edit("Materials",_up.calc_upgrade);});
...
_up.equip.info.pxp = $equip.parse.pxp(_up.equip.info.pxp2,_up.equip.info.tier);
...
_up.list = Array.from(_up.node.list.rows).map(function(tr){
    var name = tr.cells[0].textContent,
        damage = name==="Physical Damage" || name==="Magical Damage",
        data = {
            id : (/'costpane_(\w+)'/).test(tr.getAttribute("onmouseout")) && RegExp.$1,
            name : name,
            binding : _up.bindings[name],
            level : parseInt(tr.cells[1].textContent),
            forge_max : parseInt(tr.cells[3].textContent),
            upgrade_max : damage?100:50,
            upgrade_costs : damage?_up.upgrade_costs_damage:_up.upgrade_costs,
            forge_exp : 0,
            gear_exp : 0,
            costs : {},
            node : {}
        };
    data.node.input = $element("input",$element("td",tr),{type:"number",className:"hvut-up-input",value:data.level||"",min:data.level,max:data.upgrade_max},{input:function(){_up.calc_upgrade(data);}});
    data.node.table = $element("table",$id("costpane_"+data.id),[".hvut-up-sub hvut-up-costs"]);

    if(_up.max < data.level) {
        _up.max = data.level;
    }
    if(!_up.quality && /(\w+ Catalyst)/.test($id("costpane_"+data.id).textContent)) {
        var catalyst = RegExp.$1,
            catalysts = ["Wispy Catalyst","Diluted Catalyst","Regular Catalyst","Robust Catalyst","Vibrant Catalyst","Coruscating Catalyst"].slice( data.level<data.upgrade_max/4?0 : data.level<data.upgrade_max/2?1 : 2 );
        _up.quality = ["Superior","Exquisite","Magnificent","Legendary"][catalysts.indexOf(catalyst)];
    }
    return data;
});
...
_up.calc_salvage = function() {
    var text = _up.node.salvage_equip.value.trim();
    text = text.replace(/( (?:of|the))\n/g,"$1 ").replace(/\n((?:Of|The) )/g," $1");
    if(!text) {
        return;
    }
    var equip = text.split("\n").reverse().find(t=>$equip.parse.name(t).info.category!=="Obsolete");
    if(!equip) {
        alert("Couldn't find the name of the equipment.");
        return;
    }
    equip = $equip.parse.name(equip);
    var quality,pxp;
    if(_up.node.salvage_quality.value) {
        quality = _up.node.salvage_quality.value;
    } else if(/Potency Tier: (\d+) \(\d+ \/ (\d+)\)/.test(text)) {
        pxp = $equip.parse.pxp(+RegExp.$2,+RegExp.$1);
        quality = pxp>=348?"Legendary" : pxp>=335?"Magnificent" : pxp>=313?"Exquisite" : "Superior";
    } else if(["Superior","Exquisite","Magnificent","Legendary"].includes(equip.info.quality)) {
        quality = equip.info.quality;
    } else if(equip.info.quality === "Peerless") {
        quality = "Legendary";
    } else {
        quality = "Superior";
    }
    var list = Array.from(text.matchAll(/([\w ]+) Lv\.(\d+)/g)).map(function(m){
        var name = m[1].trim(),
            damage = name==="Physical Damage" || name==="Magical Damage",
            data = {
                name : name,
        ...
        return data;
    }).filter(data=>data.binding);

    _up.max = 0;
    _up.quality = quality;
    _up.type = ({"One-handed Weapon":"Metals","Two-handed Weapon":"Metals","Staff":"Wood","Shield":"Wood","Cloth Armor":"Cloth","Light Armor":"Leather","Heavy Armor":"Metals"})[equip.info.category];
    _up.rare = ({"Phase":"Crystallized Phazon","Shade":"Shade Fragment","Power":"Repurposed Actuator","Force Shield":"Defense Matrix Modulator"})[equip.info.type];
    _up.init_costs();

    list.forEach(function(e){
        _up.calc(e);
    });
    var {costs,cost} = _up.calc_list(list),
        return_costs = {},
        return_cost = 0,
        prices = $prices.get("Materials");
    for(let item in costs) {
        if(item.includes("Catalyst")) {
            continue;
        }
        return_costs[item] = Math.floor(costs[item]*0.9);
        return_cost += return_costs[item] * (prices[item]||0);
    }

    _up.node.salvage_summary.innerHTML = "";
    _up.node.salvage_summary.append(
        $element("li",null,equip.info.name),
        $element("li",null,"PXP Quality: "+_up.quality+(pxp?" ("+pxp+")":" ?")),
        $element("li",null,"Upgrade Cost: "+cost.toLocaleString()),
        $element("li",null,"Returns Value: "+return_cost.toLocaleString())
    );
    ...
    for(let item in costs) {
        $element("tr",_up.node.salvage_returns,[`/<td>${item}</td><td>${costs[item]||""}</td><td>${return_costs[item]||""}</td><td>${prices[item]||""}</td>`]);
    }
};
...
_up.toggle_salvage = function() {
    if(!_up.node.salvage) {
        ...
        _up.node.salvage_quality.append(
            ...
            $element("option",null,{text:"Legendary (348~)",value:"Legendary"}),
            $element("option",null,{text:"Magnificent (335~348)",value:"Magnificent"}),
            $element("option",null,{text:"Exquisite (313~335)",value:"Exquisite"}),
            $element("option",null,{text:"Superior (~313)",value:"Superior"})
        );
        $element("input",left,{type:"button",value:"Calculate"},_up.calc_salvage);
...
GM_addStyle(`
    ...
    .hvut-up-salvage textarea {display:block; width:380px; height:450px; margin:10px auto; font-size:10pt}
    .hvut-up-salvage ul {margin:0 0 10px; padding:0; list-style-position:inside; text-align:left; line-height:1.6em}
