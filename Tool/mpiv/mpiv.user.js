...
// @author       carry0987, kuehlschrank
// @namespace    https://github.com/carry0987
// @homepage     https://carry0987.github.io/UserJS/Tool/mpiv
...
var d = document,
    ...
    trusted = ['greasyfork.org', 'w9p.co', 'carry0987.github.io'],
    imgtab = d.images.length == 1 && d.images[0].parentNode == d.body && !d.links.length,
    cfg = loadCfg(),
...
function loadHosts() {
    var hosts = [
        ...
        { r: /imgur\.com\/(a|gallery|t\/[a-z0-9_-]+)\/([a-z0-9]+)(#[a-z0-9]+)?/i, s: function(m) { return 'https://imgur.com/' + m[1] + '/' + m[2] + '' + (m[3] || ''); }, g: function(text, url, cb) { var mk = function(o, imgs) { var items = []; if (!o || !imgs) return items; for (var i = 0, len = imgs.length, cur; i < len && (cur = imgs[i]); i++) { var iu = 'https://i.imgur.com/' + cur.hash + cur.ext; if (cur.ext == '.gif' && !(cur.animated === false)) iu = [iu.replace('.gif', '.webm'), iu.replace('.gif', '.mp4'), iu];
                            items.push({ url: iu, desc: cur.title && cur.description ? cur.title + ' - ' + cur.description : (cur.title || cur.description) }); } if (o.is_album && !contains(items[0].desc, o.title)) items.title = o.title; return items; },
                    m = /(mergeConfig\('gallery',\s*|Imgur\.Album\.getInstance\()(\{[\s\S]+?\})\);/.exec(text),
                    o1 = eval('(' + m[2].replace(/analytics\s*:\s*analytics/, 'analytics:null').replace(/decodeURIComponent\(.+?\)/, 'null') + ')'),
                    o = o1.image || o1.album,
                    imgs = o.is_album ? o.album_images.images : [o]; if (!o.num_images || o.num_images <= imgs.length) return mk(o, imgs);
                GM_xmlhttpRequest({ method: 'GET', url: 'https://imgur.com/ajaxalbums/getimages/' + o.hash + '/hit.json?all=true', onload: function(res) { var imgs; try { imgs = JSON.parse(res.responseText).data.images; } catch (ex) {} cb(mk(o, imgs)); } }); }, css: '.post > .hover { display:none!important; }' },
        ...
        { r: /(min\.us|minus\.com)\/m[a-z0-9]+$/i, g: function(text) { var m = /gallerydata = ({[\w\W]+?});/.exec(text),
                    o = JSON.parse(m[1]),
                    items = [];
                items.title = o.name; for (var i = 0, len = o.items.length, cur; i < len && (cur = o.items[i]); i++) { items.push({ url: 'https://i.minus.com/i' + cur.id + '.jpg', desc: cur.caption }); } return items; } },
...
function onKeyUp(e) {
    switch (e.keyCode) {
        case 16:
            ...
            if (_.controlled) { _.controlled = false };
...
function loadGalleryParser(g) {
    ...
    return function(text, url) {
        var qE = g.entry,
            ...
            qI = g.image,
            qT = g.title,
            fix = (typeof g.fix == 'string' ? new Function('s', 'isURL', g.fix) : g.fix) || function(s) { return s.trim(); };
            ...
            try {
                ...
                if (!type) {
                    var ext = /\.([a-z0-9]+?)($|\?|#)/i.exec(url) ? RegExp.$1.toLowerCase() : 'jpg',
                        types = { bmp: 'image/bmp', gif: 'image/gif', jpe: 'image/jpeg', jpeg: 'image/jpeg', jpg: 'image/jpeg', mp4: 'video/mp4', png: 'image/png', svg: 'image/svg+xml', tif: 'image/tiff', tiff: 'image/tiff', webm: 'video/webm' };
...
function checkProgress(start) {
    ...
    if (_.preloadStart) {
        ...
        if (wait > 0) {_.timeout = wn.setTimeout(checkProgress, wait)};
    }
...
function updateScales() {
    var scales = cfg.scales.length ? cfg.scales : ['0!', 0.125, 0.25, 0.5, 0.75, 1, 1.5, 2, 3, 5, 8, 16],
        fit = Math.min((_.view.width - _.mbw) / _.nwidth, (_.view.height - _.mbh) / _.nheight),
...
function setPopup(src) {
    ...
    if (src.substr(0, 5) != 'data:' && /\.(webm|mp4)($|\?)/.test(src) || src.substr(0, 10) == 'data:video') {
        ...
        on(p, 'canplaythrough', function(e) { off(e.target, 'progress', onProgress); if (_.bar && _.bar.classList.contains('mpiv-xhr')) { setBar(false);
                showFileInfo(); } });
    } else {
        p = _.popup = ce('img');
    }
...
function matches(n, q) {
    var p = Element.prototype,
        m = p.matches || p.mozMatchesSelector || p.webkitMatchesSelector || p.oMatchesSelector;
